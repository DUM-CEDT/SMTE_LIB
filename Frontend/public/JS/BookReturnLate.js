...
async function GetAllLendingBook() {
    ...
    for (let i = AllLendingHistory.length - 1; i >= 0; i--) {
        ...
        let thisbook = await(await fetch(BACKEND_URL + "/API/Book/" + thishistory.Book_ID)).json()
        let thisstudent = await(await fetch(BACKEND_URL + "/API/Student/" + thishistory.Student_ID)).json()
        let listt = [thisbook.Book_Name, thisstudent.Student_Name , thishistory.Lend_Date.Date.toString() + "/" + thishistory.Lend_Date.Month.toString() + "/" + thishistory.Lend_Date.Year.toString() , thishistory.Must_Return_Date.Date.toString() + "/" + thishistory.Must_Return_Date.Month.toString() + "/" + thishistory.Must_Return_Date.Year.toString()]
        for (let j = 0; j < 4; j++) {
